...
class AgentFrameworkProductManagementAgent:
    ...
    agent: ChatAgent
    session: AgentThread = None
    SUPPORTED_CONTENT_TYPES = ['text', 'text/plain']

    def __init__(self):
        # Configure the chat completion service explicitly
        chat_service = get_chat_client()
        # Map of session_id -> AgentThread (local message-store threads).
        # We don't pass session_id to Azure as conversation_id because
        # the Chat Completions API doesn't support it.
    ...
    async def _ensure_session_exists(self, session_id: str) -> None:
        ...
        Args:
            session_id (str): Unique identifier for the session.
        """
        if self._current_session_id != session_id or self.session is None:
